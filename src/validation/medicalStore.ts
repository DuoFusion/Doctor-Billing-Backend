...
export const addMedicalStoreValidation = Joi.object({
  ...
  defaultCompanyAddress: Joi.string().trim().allow("").max(500).optional(),
  defaultCompanyCity: Joi.string().trim().allow("").max(100).optional(),
  defaultCompanyState: Joi.string().trim().allow("").max(100).optional(),
  defaultCompanyPincode: pincodeField.allow("").optional(),
